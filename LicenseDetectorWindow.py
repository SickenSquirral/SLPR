...
class Vehicle:
    def __init__(self, data):
        if data is None:
            return
        self.licence_plate = data['kjoretoydataListe'][0]['kjoretoyId'].get('kjennemerke')
...
def get_vehicle_info(licence_plate):
    headers = {
        'SVV-Authorization': f'Apikey {API_KEY}'
    }
    response = requests.get(API_URL + licence_plate, headers=headers)
...
def licence_plate_formatter(string):
    ...
    while(True):
        ...
        for i in range(len(scores)):
            if ((scores[i] > min_conf) and (scores[i] <= 1.0)):
                ...
                label = '%s: %d%%' % (licence_plate, int(scores[i]*100)) 
                ...
                if license_plate_text != last_license_plate_text and current_time - last_detection_time > detection_interval:
                    licence_plate = licence_plate_formatter(license_plate_text.result())
                    if licence_plate:
                        # Increment the count for this license plate
                        if licence_plate in license_plate_counts:
                            license_plate_counts[licence_plate] += 1
                        else:
                            license_plate_counts[licence_plate] = 1

                        # Only make the API call if this license plate has been detected three times
                        if license_plate_counts[licence_plate] >= 3 and (not newest_vehicle or licence_plate != newest_vehicle.licence_plate):
                            vehicle_info = get_vehicle_info(licence_plate)
                            if vehicle_info:  # Check if vehicle_info is not empty
                                newest_vehicle = Vehicle(vehicle_info)
                                print(newest_vehicle.__dict__)
                            last_license_plate_text = licence_plate
                            last_detection_time = current_time
                
                # Display vehicle attributes on the video feed
                if newest_vehicle:
                    vehicle_attributes = f"License Plate: {newest_vehicle.licence_plate}\nType: {newest_vehicle.type}\nBrand: {newest_vehicle.brand}\nFuel: {newest_vehicle.fuel}\nDrivetrain: {newest_vehicle.drivetrain}\nMax Speed: {newest_vehicle.max_speed}\nPower: {newest_vehicle.power}\nWeight: {newest_vehicle.weight}\nFirst Registration: {newest_vehicle.first_registration}"
                    y0, dy = 50, 30
                    for i, line in enumerate(vehicle_attributes.split('\n')):
                        y = y0 + i*dy
                        cv2.putText(frame, line, (50, y ), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
